...
def loss_funtion(num_of_samples, loss_name):
  if(not num_of_samples.isnumeric()):
    ...
    for i in range(n):
      ...
      if loss_name == "MAE":
        print(f"loss_name: {loss_name},sample:{i},pred:{predict},target:{target},loss:{abs(predict - target)}")
      elif loss_name == "MSE":
        print(f"loss_name: {loss_name},sample:{i},pred:{predict},target:{target},loss:{(predict - target)**2}")
      elif loss_name == "RMSE":
        print(f"loss_name: {loss_name},sample:{i},pred:{predict},target:{target},loss:{math.sqrt((predict - target)**2)}")
